...
Run from anywhere (paths anchored on `__file__`): `uv run python scripts/z_generate_manuscript_variables.py` from this project root, or `uv run python projects/cogant/scripts/z_generate_manuscript_variables.py` from the parent template root after vendoring.
