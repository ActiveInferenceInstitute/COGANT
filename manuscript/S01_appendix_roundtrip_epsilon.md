...
> **Roundtrip evidence banner.** The shipped
> `cogant/evaluation/dataset/roundtrip_results.jsonl` is a **native v0.6
> ledger** regenerated by `tools/regenerate_roundtrip_ledger.py`: every row
> carries native `role_preservation_score` and `roundtrip_status` fields
> parsed by `tools/regenerate_metrics.py`, so the aggregates below are fresh
> v0.6 measurements (score source `{{ROLE_PRESERVATION_SCORE_SOURCE}}`), not
> legacy ε proxies. The wave-14/16 snapshots cited later in this appendix are
> historical development milestones retained for provenance only.
...
| Retained legacy epsilon archive | v0.5 `tier`, `epsilon`, size fields; no native v0.6 status | {{STALE_LEGACY_COUNT}} STALE_LEGACY (superseded by the native ledger) | Historical context only |
...
> which runs `verify_repo_roundtrip` (forward → reverse → forward) on every
> shipped fixture and writes per-row `role_preservation_score` and
> `roundtrip_status`. On this native ledger `METRICS.yaml` reports
> **{{ROLE_PRESERVED_COUNT}}** role-preserved of **{{TOTAL_TARGETS}}** targets
> (mean `role_preservation_score` **{{MEAN_ROLE_PRESERVATION_SCORE}}**, score
> source `{{ROLE_PRESERVATION_SCORE_SOURCE}}`), **{{STRICT_ISOMORPHISM_COUNT}}**
> strictly structurally isomorphic, and **{{DRIFT_COUNT}}** drift; the
> {{STALE_LEGACY_COUNT}} `STALE_LEGACY` rows of the prior ε-corpus are fully
> superseded. `tools/check_metrics_fresh.py` re-derives this distribution from
> the same ledger as an anti-laundering gate. See @sec:08-05-threats-to-validity
> "Native v0.6 roundtrip ledger" for the construct-validity bound (role recall,
> not isomorphism; in-sample fixtures).
>
> **The wave tables below are historical** development snapshots, retained for
> provenance only and superseded by the native ledger above. Wave 14 is the
> CONSTRAINT-synthesizer fix; wave 16 (2026-04-10) added the POLICY + CONTEXT
> synthesizer fixes; see @sec:S01-appendix-a2-constraint for the CONSTRAINT fix
> trajectory and `../cogant/docs/evaluation/ROUNDTRIP_IMPROVEMENT.md` for the
> full wave-14 → wave-16 trajectory.
...
`../cogant/docs/evaluation/ROUNDTRIP_IMPROVEMENT.md`. These wave snapshots are
historical; the checked-in `roundtrip_results.jsonl` ledger has since been
regenerated **natively** by `tools/regenerate_roundtrip_ledger.py`, and
`METRICS.yaml` now classifies its rows from per-row `role_preservation_score`
and `roundtrip_status` fields (score source `{{ROLE_PRESERVATION_SCORE_SOURCE}}`,
{{STALE_LEGACY_COUNT}} `STALE_LEGACY`). The native aggregate counts are
**{{ROLE_PRESERVED_COUNT}}** ROLE_PRESERVED, **{{STRICT_ISOMORPHISM_COUNT}}**
STRUCTURALLY_ISOMORPHIC, **{{DRIFT_COUNT}}** DRIFT, and **{{FAILED_COUNT}}**
FAILED of **{{TOTAL_TARGETS}}** (mean role-preservation score
