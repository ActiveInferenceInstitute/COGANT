...
### Definition: Evidence-labelled assertion {#def:evidence-labelled-assertion}
...
### Definition: Translation rule {#def:translation-rule}
...
- $p_r \in \mathbb{Z}$ is the rule priority consulted during conflict resolution (@alg:conflict-resolution).
...
### Definition: Fixpoint semantics {#def:fixpoint-semantics}
...
computed iteratively by `TranslationEngine.translate()` and then post-processed by `_resolve_conflicts()` (@alg:conflict-resolution). The post-processing step applies an anti-monotone pruning to the fixpoint, and is therefore specified outside of $F_{G,R}$ rather than folded into it.

### Definition: Markov blanket partition {#def:markov-blanket-partition}
...
### Definition: A/B/C/D matrices {#def:abcd-matrices}
...
Let $G = (V, E, \lambda_V, \lambda_E, \tau)$ be a finite program graph with $|V| = n$, let $R$ be a finite rule set with $|R| = k$, and let $F_{G,R}$ be the rule-application operator of @def:fixpoint-semantics. Then the Kleene chain
...
### Implementation invariant: Markov blanket partition totality {#inv:markov-blanket-partition-totality}

For any program graph $G = (V, E, \lambda_V, \lambda_E, \tau)$ with $V \neq \emptyset$ and any seed set $S \subseteq V$, the seed-induced structural partition $\Pi_{G, S}$ of @def:markov-blanket-partition is **total** (every $v \in V$ receives exactly one role) and **mutually exclusive** (no $v$ is assigned two roles). This invariant is about the implemented partitioner, not about probabilistic Markov-blanket semantics.
...
### Proposition: Matrix validity {#prop:matrix-validity}

If $|\mathbf{O}| \geq 1$, $|\mathbf{V}| \geq 1$, and $|\mathbf{A}| \geq 1$, then the matrices $(A, B, C, D)$ produced by `GNNMatrices.compute_A/B/C/D` satisfy the stochastic conditions of @def:abcd-matrices within a numerical tolerance of $10^{-6}$.
