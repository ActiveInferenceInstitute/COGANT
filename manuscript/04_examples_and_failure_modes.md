...
![A/B/C/D matrix panel for the richer Flask application fixture. The heatmaps are rendered from exported `model.gnn.json` arrays: A shows likelihood values with non-uniform state columns, C shows the real zero preference vector, D shows the exported prior vector, and B shows the recorded max-over-actions summary of the exported transition tensor. Panel annotations report source/display shapes, distinct values, nonzero cells, and B identity-slice diagnostics so real structure and weak-evidence regions are visible before downstream inference.](../figures/cogant_forward_abcd_matrices.png){#fig:cogant-abcd-matrices width=98%}
