...
![Publication timeline for the calculator batch run, rendered from `cogant/output/run_manifest.json`. The ordered bars show the recorded `run_all.py` stages and their local wall-clock durations, including the explicit `roundtrip:calculator` stage after forward translation, export, rendering, visualization, and validation; dark diamond markers distinguish validation and roundtrip gates from ordinary execution stages. Use the timeline as provenance that the roundtrip step executed and wrote `roundtrip/forward/model.gnn.md` plus a synthesized Python package under `roundtrip/reverse/calculator/`; do not read the single-run durations as benchmark results.](../figures/cogant_roundtrip_batch_gantt.png){#fig:cogant-roundtrip-gantt width=95%}
...
![Roundtrip visual diff for the calculator fixture. The panel is generated from `roundtrip/metrics.json` and shows, as bars, the original-versus-regenerated node and edge count deltas, the normalized graph-edit distance, the added and removed GNN sections, and the maximum absolute matrix delta; below the bars it lists per-invariant pass/check status and a status line that separates the strict-structure, role, matrix, GNN, and generated-code checks. Read role preservation as a weaker success tier than strict structural isomorphism.](../figures/cogant_roundtrip_visual_diff.png){#fig:cogant-roundtrip-visual-diff width=95%}
