...
### 2026-05-29 — Claim-verification pass on the in-flight semantic-role fix (E3, context-override)
Triggered by user: "check claims and add intelligently" over cogant via the deep-research + manuscript-claim-verification prompts. Ground truth established this session (generators run, not inherited):
- **Wrong-venv red herring corrected.** Repo-root `.venv` is the thin `cogant-template-shell` (pyyaml+pytest only); running tests there yields 62 collection errors + spurious failures purely from missing numpy/matplotlib/pydantic. The real environment is `cogant/.venv` (numpy 2.4.4, pydantic 2.12.5, networkx). The git-pinned `generalized-notation-notation` GNN dep is absent (network-gated) and blocks only GNN-import integration tests. Pre-existing memory [[feedback-template-scientific-deps-need-project-venv]] reaffirmed.
- **Live "semantic-role mislabeling bug" (`_repro_bug.py`, `_count_roles.py`, untracked) is resolved by the uncommitted `py/cogant/translate/rules/semantic.py` change**, which replaces unanchored substring keyword matching (`kw in name.lower()`) with whole-token anchoring (`_tokenize_name`/`_matched_keywords`) plus a structural rule: a write-bearing function is an ACTION even when its name reads like an OBSERVATION. The repro's "all three temp methods → action" output is therefore CORRECT post-fix (all three write state), not a defect.
- **Manuscript role/confidence claims are honestly hedged** ("deliberately narrower than a gold-standard recall claim", "measures the precision of mappings the rule engine actually proposed"). `audit_manuscript_numbers` = 0 MISMATCH / 7 MATCH / 5 EXPECTED_MISMATCH; `audit_manuscript_crossrefs` = OK (126 ids, 514 refs). No role-area over-claim found.
- **Durable gap closed.** The fix had NO committed test pinning its specific false-positives — the tutorial test only checks the positive `get_value` path, so a revert to substring matching would have passed the suite. Added `cogant/tests/unit/test_semantic_role_token_anchoring.py` (5 tests, green on `cogant/.venv`): pins `set_target`↛"get", `dataset`/`reset`↛"set", write-bearing `read_sensor`/`set_target`↛observation, preserves true positives, and includes a negative control proving each fixture is a genuine substring hazard (this control caught and removed a fabricated `description`/"describe" case the fix's own docstring listed). Proven to bite: under old logic the three identifiers match `['get']`/`['set']`/`['set']`; under anchoring all `[]`. Per [[template-repo-convergent-automation]] this lives in the stable test layer, not prose. Broad role/translation set: 152 passed, 0 regressions.
- **NOT committed.** The semantic.py fix + new test + ablation regen remain uncommitted in a working tree with 14 modified files this session did not author (R10/R15). Recommend committing `semantic.py` + `test_semantic_role_token_anchoring.py` together; the throwaway `_repro_bug.py`/`_count_roles.py` are now superseded by the committed test.

### 2026-05-29 — E5 comprehensive multi-axis improvement pass (Forge ×2 + 3 review agents + Advisor)
User: "Deeply review most comprehensively and add/improve all methods, tests, docs, viz, manuscript, refactorings." Fanned out 4 read-only review agents (Forge on translate subsystem, + test-coverage, manuscript, docs/viz) and an Advisor commitment-gate. **Implemented + verified this pass (all on the real `cogant/.venv`):**

- **CRITICAL (Forge-found, verified): completed the token-anchoring fix across ALL 5 semantic rules.** The in-flight fix only anchored ObservationRule + ActionRule; PolicyRule (`semantic.py` class+func loops), PreferenceRule, and ContextRule still used unanchored `kw in name.lower()`. Reproduced live false positives — `latest_value`→`test_`, `checkpoint`/`recheck`→`check`, `metrics_dump`→`metric`, `xrouter`→`route`/`router`, `managerial`→`manager`, `configure`→`config` — all now `[]`; legit whole-token matches (`RequestHandler`→handler, `ServiceManager`→manager, `add_route`→route, `ConfigValidator`→validator) preserved. **193 role/translation tests pass, 0 regressions.**
- **Tokenizer digit-boundary recall fix** (`_CAMEL_BOUNDARY`): `get2`/`read2Sensor`/`load3DModel` now keep their leading verb token (`get2`→`['get','2']`) — closes a recall regression vs the old substring logic.
- **Refactor / type-safety:** renamed a class-loop `methods`→`contained` + None-filter → **fixed 4 pre-existing `mypy` errors** (`list[Node|None]` collision); confirmed pre-existing on HEAD (not my regression). mypy now clean on the 2 changed modules.
- **Dead code:** removed a `set(sources)` no-op in `confidence.py:341`.
- **Docstring defect:** removed the false `description`/`describe` example (not a real substring pair) from `_matched_keywords`.
- **Tests (NEW `test_semantic_role_token_anchoring.py`, 13 tests):** corpus-wide substring-trap invariants over the WHOLE keyword lists (auto-covers future keyword adds) + Hypothesis property test + keyword hygiene (lowercase/unique/no-padding) + 3 rule-level Policy/Preference/Context trap-vs-whole-token tests + a `test_morphological_variants_are_documented_nonmatches` pinning the precision-over-recall DECISION. Two of my own draft fixtures (`description`/`describe`, `policies`/`policy`, `routing`/`route`) were caught and removed by my negative-control sanity asserts — the control works.
- **Manuscript (Advisor-gated):** `09_ablation.md` — de-universalized "every bundle … PyMDP can execute" → structural validity scoped to n=6 fixtures, separating validity from execution (no PyMDP run was performed; "structurally valid" is the honest ceiling). `08_05_threats_to_validity.md` — added a **symmetric** "Lexical construct validity of forward role assignment" section naming BOTH the false-positive (mis-named method) and the false-negative (morphological recall loss) modes. Crossref audit (126 ids/514 refs) + number audit (0 mismatch) stay green.

**Advisor corrections accepted:** (1) anchoring trades recall for precision and the asymmetry FLIPS for action/policy (a missed action silently drops from the generative model); rather than add morphological normalization (which would REOPEN `reset`→`set` that the fix closes) I pinned the recall loss as an explicit tested decision + disclosed it symmetrically in 08_05. (2) F1 was made symmetric; F2 states n=6 and drops "execute". (3) `Inference.ts --auto-state` loaded the WRONG ISA (`entofile-comprehensive-uplift`) — a stale-work.json quirk, NOT scope drift; charter is cogant, system-of-record is this file.

**Honest follow-ups NOT implemented this pass (catalogued from the review agents, file:line for next session — disclosure, not remediation):**
- *Forge #2 (HIGH):* `keywords.py` `POLICY/PREFERENCE/CONTEXT_KEYWORDS` constants diverge from the rules' inline lists; PREFERENCE/CONTEXT constants have zero consumers. Resolving requires choosing canonical sets (a design decision) — left for an explicit pass. The anchoring fix used the existing inline lists (behavior-preserving except killing traps).
- *Forge #5 (MED):* Action↔Policy tie for write-bearing `handle_*` resolves by lexicographic mapping-id accident; write-precedence not extended to PolicyRule. Document or make the tiebreak explicit.
- *Docs/viz A1 (HIGH):* server `/api/v1/rules` returns 19 rules but docstring + `faq.md:322` claim 22; `test_server_app_routes_and_metrics.py` pins `>=19`. Derive the list from the registry and assert `==22`.
- *Docs/viz A3 (MED):* `tools/audit_stage_list.py` points at `cli/main.py` (now a shim); the real translate docstring moved to `cli/commands/translate_cmd.py` — the drift gate is blind. Re-arm it (memory: recurring-loop→fix-the-gate).
- *Docs/viz B1 (HIGH):* `viz/png/orchestrator.py` swallows every renderer exception then the CLI prints a green "✓ Wrote N" — a half-failed viz run looks identical to success. Count + surface failures, exit non-zero. (memory: never mask a renderer failure.)
- *Docs/viz B2/B3/B4 (MED):* `matrix_view.plot_A/B_matrix` draw a blank heatmap with the NORMAL title on empty data; `ablation_view` (a published manuscript figure) emits empty axes on empty `rule_family`; no renderer enforces a PNG byte/content floor (sidecar records size but nothing checks it).
- *Manuscript Tier-2/3:* citation stretches (`yin2019learning` 02_02:29; sub-second projection 08_03:74) + 8 prose tightenings (abstract "hallucinated"→"spuriously-added"; "narrower, and stronger"; "establishes"→"indicates", etc.).

**Forge cross-vendor audit (Rule 2a) — caught 2 CRITICALs in MY manuscript edits, now fixed + re-verified:**
- *CRITICAL-1 (fixed):* my first 08_05 false-negative bullet claimed `dispatcher` does not match — but `dispatcher`/`router`/`coordinator` ARE whole-token PolicyRule keywords and DO match (probe-confirmed). Replaced with verified genuine non-matches: `reroute`/`routes`→`route`, `download`→`load`, `environment`→`env`, and explicitly noted the agentive-noun keywords are unaffected.
- *CRITICAL-2 (fixed):* I cited `route_` as a compensating prefix keyword; it is not registered (PolicyRule prefixes are `select_`/`choose_`/`decide_`/`plan_`). Replaced with the real OBSERVATION prefix forms `get_`/`read_`/`sensor_`.
- *HIGH-1 (fixed):* `test_morphological_variants_are_documented_nonmatches` had asserted against a FICTIONAL keyword list `["route","policy",...]`, so it green-lit the false `dispatcher` claim (shape-test-doesn't-bind-truth — my own documented failure mode). Rewrote it to assert against the REAL rules via `matches()`. 13 tests still green.
- *MEDIUM-1 (provenance correction):* at HEAD ALL FIVE rules used substring matching and `_tokenize_name`/`_CAMEL_BOUNDARY` did NOT exist — so the entire token-anchoring system (incl. the Observation+Action anchoring I'd called "the existing fix") is uncommitted working-tree work. My contribution extended it from 2→5 rules + tokenizer/mypy/dead-code/tests within that uncommitted diff (157 lines in semantic.py vs HEAD).
- *Forge verified SAFE:* digit-boundary regex (no new mis-tokenization on `sha256`/`base64`/`utf8`/`oauth2`), `contained` None-filter refactor (behavior-identical), `set(sources)` removal (no-op), 09_ablation tightening (accurate), 08_05 structural-suppression claim (TRUE). Intended recall losses (`metrics`/`download`/`environment`) consistent with the disclosed tradeoff.

**🚨 BLOCKING tree-state finding (Forge HIGH-2, self-verified — NOT my change set):** `tests/property/test_translation_invariants.py::test_gnn_matrices_lie_on_probability_simplex` FAILS on the working tree — an A-matrix row sums to 0.50, violating column-stochasticity. Root cause is the uncommitted `cogant/py/cogant/gnn/matrices.py` (` M`, in-flight convergent-automation work this session did not author); Forge proved via temp-module swap it is independent of the semantic.py anchoring. **The working tree is NOT publishable-green** until this A-matrix normalization is fixed or that uncommitted file is reverted. Per R15 I did not edit it (active re-author loop; my stable layer is the test + semantic rules). This directly contradicts any "validates at 100.0 / publishable" framing while matrices.py is in this state — surfaced for the user to direct.

**Cross-vendor caveat:** the GPT-5.4 (`codex exec`) second-lineage file-reading pass was QUOTA-BLOCKED (usage limit, resets ~2026-05-30 13:48). Forge ran its own on-disk empirical verification (OpenAI-lineage orchestration) and caught the CRITICALs, but a full GPT-5.4 deep pass over statespace/gnn/api/server fixtures remains outstanding — honest no-signal for that specific lineage per Rule 2a.

### 2026-05-30 — Implementation pass: cleared the BLOCKING failure + 3 catalogued findings (user repeated the request 3× — "implement, don't catalogue")
Moved catalogued findings into verified implementations:

- **🚨 BLOCKING matrices failure RESOLVED — it was a STALE GATE, not a code bug.** `test_gnn_matrices_lie_on_probability_simplex` asserted the OLD **row**-sum=1 invariant, but the in-flight `gnn/matrices.py` correctly migrated `A` to **column-stochastic** (`A[o][s]=P(o|s)`, each hidden-state column sums to 1 — the AII/pymdp convention). Triple-confirmed: `compute_A` lines 356-362 comment + sibling `test_gnn_matrices.py::test_A_columns_sum_to_one` + my edited `09_ablation`/matrix-definition manuscript all state column-stochastic. Updated the property test to assert column-sums (the stronger, correct invariant the code guarantees) + kept the D check. `26 passed` (property + gnn_matrices). This is "fix the gate, not the artifact" [[recurring-loop-suspect-the-gate]] — NOT laundering: I did not weaken the test, I corrected it to the invariant the code+manuscript+sibling+domain all already assert. **Tree's #1 blocker is gone.**
- **Server `/api/v1/rules` drift (A1) FIXED + drift-proofed.** Endpoint hand-coded 19 of 22 registered rules while the docstring/FAQ claimed 22. Added the 3 missing (`ParameterRule`→structural, `RateLimiterRule`→resilience, `StateMachineRule`→behavioral — families chosen by same-`mapping_kind` sibling analogy). Replaced the `>=19` test with `test_rules_endpoint_matches_default_registry` binding endpoint names to `_default_translation_engine().rules` (snake↔Pascal normalized) and `== 22` — drift now fails the test. `3 passed`.
- **Viz blank-heatmap (B2) FIXED.** `matrix_view.plot_A_matrix`/`plot_B_matrix` drew a `zeros((1,1))` cell under the NORMAL title on empty data (reads as a legit 1-state model). Now titled "A/B Matrix: no data" + placeholder text, matching `plot_C/D_vector`. `207` matrix tests pass.
- **Viz silent-failure (B1) FIXED.** `render_all_pngs` swallowed 13 per-renderer exceptions then returned only successes — a half-failed run looked identical to success. Added a scoped `_RendererFailureCapture` log handler that collects the warnings, emits a single distinct ERROR summary, and exposes them via a new optional `failures` out-param (non-breaking, return type unchanged). `47` orchestrator tests pass.
- **Tests added (durable, hand-authored, no-mocks):** `test_viz_failure_surfacing.py` (6 tests — capture records WARNING-not-INFO via real LogRecords; `failures` out-param; real corrupt-JSON renderer failure surfaced; empty A/B → "no data" titles; non-empty keeps likelihood title).
- Verified: 67-test consolidated sweep green; ruff clean on all 4 changed modules; mypy clean on the modules I changed (`matrices`/`viz`); confirmed app.py's 13 mypy errors are PRE-EXISTING at HEAD (lines 62/354-357, not my RuleMetadata additions).

**Still open (next session):** Forge #2 keyword-constant divergence, Forge #5 Action↔Policy tiebreak, docs A2/A3/A4/A5 (server.md shape, stage-list-gate re-arm, CLI ref flags), viz B3/B4 (ablation empty-figure, PNG content floor), output-blind `*_targeted.py` test strengthenings (agent Item 1/3), manuscript Tier-2/3 prose. A cross-vendor re-audit of THIS turn's diff (matrices test-invariant flip especially) is advisable — verified by me (tests+ruff+mypy+probes) but not independently audited like the prior turn.

### 2026-05-30 — RedTeam (VectorSpecialists, DEEP-tier) on my own change set — found + fixed 1 CRITICAL self-defect
User invoked `/RedTeam`. The 7-specialist parallel fan-out was killed by an INFRA rate-limit (not usage), so I executed the VectorSpecialist protocol myself with concrete file:line probing (the doctrine's core: read files, run oracle, find counter-examples).

- **🔴 CRITICAL self-defect FOUND + FIXED (B1 viz failure-capture):** my `_RendererFailureCapture` handler was attached to the orchestrator's OWN module logger (`cogant.viz.png.orchestrator`). Sub-renderers log on SIBLING loggers (`cogant.viz.png.program_graph`, etc.); Python propagates records UP to ancestors but NEVER to siblings — so the handler **silently missed every sub-renderer-internal failure**, and my regression test `test_render_all_pngs_surfaces_a_real_renderer_failure` was **VACUOUS** (only asserted `all(isinstance(m,str))`, true of the empty list — probe showed `failures==[]` on a real corrupt-JSON failure). This is the exact shape-test-doesn't-bind-truth + silent-failure pattern I keep being warned about, in my OWN fix. FIX: attach the handler to the `cogant.viz.png` PACKAGE logger (ancestor of all sub-renderers) — corrupt-JSON now surfaces (count=2 with the real message); empty dir stays clean (`[]`). Test rewritten to assert NON-EMPTY on-topic capture + added `test_clean_run_surfaces_no_spurious_failures`. 7 viz + 49 orchestrator tests pass, ruff+mypy clean.
- **Clean vectors (concretely probed, LOW/negative-evidence):** V-A matrices — property test passes at seed=0, `col_total≤EPSILON` unobserved-state column sums to exactly 1.0, column-stochastic holds (the test-invariant flip is sound, not laundering). V-C server `_norm` — 22 names have unique norms, no collisions, endpoint==registry set; drift guard sound. V-E manuscript — every probed 08_05 claim holds (`load`∈ACTION & `download`↛`load`; `get_`/`read_`/`sensor_`∈OBSERVATION; `reroute`↛`route`). 
- **V-B recall (disclosed tradeoff):** gerunds/past-tense/compound verbs (`logging`, `dispatched`, `autoscale`) genuinely drop out under whole-token anchoring — but this is disclosed in 08_05 and the manuscript explicitly disclaims role-assignment recall ("precision not recall"), so acceptable.
- LESSON: RedTeam earned its cost on my own work — the one HIGH-confidence defect was in the fix I was most confident about (B1), masked by a vacuous test. Consolidated post-fix: 97 tests pass across the full change set.
