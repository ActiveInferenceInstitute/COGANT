...
``gnn_validation_report.json`` files, and writes:
    ...
    _rnd/figures/fig1_graph_sizes.png        -- bar chart of nodes/edges per fixture
    _rnd/figures/fig2_node_kinds.png         -- stacked bar of node kinds per fixture
    _rnd/figures/fig3_state_space.png        -- bar chart of state-space outputs per fixture
    _rnd/figures/fig4_pipeline_latency.png   -- bar chart of wall-clock time per fixture
    _rnd/figures/metrics.json                -- machine-readable metrics (manuscript source of truth)
    _rnd/figures/metrics_table.md            -- markdown table mirroring the manuscript

Run from the ``projects_in_progress/cogant/`` directory::

    python _rnd/figures/generate_figures.py
