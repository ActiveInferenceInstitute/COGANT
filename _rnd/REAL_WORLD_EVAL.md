...
**Raw metrics:** `_rnd/real_world_eval_summary.json`
...
pre-existing `_rnd/real_world_eval_results.json`) reported the same node/edge
...
for repo in flask requests httpx fastapi pydantic click rich dulwich; do
  /usr/bin/time -l \
    uv run cogant translate ../_rnd/eval_repos/$repo \
...
- Collected metrics JSON: `_rnd/real_world_eval_summary.json`
