...
* ``_rnd/dataset/instances/{name}.json`` -- one file per repo, carrying
  the full nodes/edges/mappings/A-B-C-D matrices bundle.
* ``_rnd/dataset/nodes.jsonl`` -- one JSON-per-line record per program
...
* ``_rnd/dataset/instances.jsonl`` -- one JSON-per-line record per repo
...
# ``cogant/_rnd/dataset/generate_dataset.py`` and the package lives at
# ``cogant/cogant/py/cogant/`` relative to the project root.
PROJECT_ROOT = Path(__file__).resolve().parents[2]
PACKAGE_ROOT = PROJECT_ROOT / "cogant" / "py"
...
# Fixtures live under cogant/examples/. Paths are resolved relative to
# PROJECT_ROOT (the cogant/ sub-tree inside projects_in_progress).
FIXTURES: list[tuple[str, str]] = [
    ("calculator", "cogant/examples/control_positive/calculator"),
    ("event_pipeline", "cogant/examples/control_positive/event_pipeline"),
    ("flask_mini", "cogant/examples/control_positive/flask_mini"),
    ("flask_app", "cogant/examples/real_world/flask_app"),
    ("requests_lib", "cogant/examples/real_world/requests_lib"),
    ("json_stdlib", "cogant/examples/real_world/json_stdlib"),
]
...
def main() -> int:
    ...
    dataset_dir = PROJECT_ROOT / "_rnd" / "dataset"
    ...
    print(f"  _rnd/dataset/dataset_summary.json")
    print()
