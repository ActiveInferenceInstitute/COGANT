...
| `generate_dataset.py`  | Deterministic generator script (rerun with `python evaluation/dataset/generate_dataset.py`). | - |
...
# Node-level
nodes = [json.loads(line) for line in open("evaluation/dataset/nodes.jsonl")]
train_nodes = [n for n in nodes if n["split"] == "train"]
print(f"train nodes: {len(train_nodes)}")

# Instance-level
instances = [json.loads(line) for line in open("evaluation/dataset/instances.jsonl")]
for inst in instances:
    print(inst["repo_id"], inst["graph"]["node_count"], inst["gnn"]["A_shape"])

# Full per-repo bundle (nodes + edges + GNN shape)
with open("evaluation/dataset/instances/calculator.json") as f:
    bundle = json.load(f)
