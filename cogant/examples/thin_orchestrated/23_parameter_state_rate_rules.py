#!/usr/bin/env python3
"""Thin example: parameter, state, and rate-limit translation rules (ParameterRule, StateMachineRule, RateLimiterRule).

This script builds a synthetic ProgramGraph with patterns that specifically
trigger the three "parameter/state/rate-limit" rules from the resilience family:
...
Run from the repo root:
    ...
    PYTHONPATH=py python examples/thin_orchestrated/23_parameter_state_rate_rules.py \\
        --output-dir output/thin/parameter_state_rate_rules
"""
...
def _build_synthetic_graph():
    """Build a synthetic ProgramGraph with patterns for the three parameter/state/rate-limit rules."""
    builder = ProgramGraphBuilder(repo_uri="synthetic://parameter_state_rate_demo")
...
def main() -> int:
    """Entry point for the parameter/state/rate-limit rules demo."""
    args = parse_args("parameter_state_rate_rules")
    configure_logging()
    banner("Stage 23: parameter, state, and rate-limit translation rules")
    ...
    # Register only the three parameter/state/rate-limit rules
