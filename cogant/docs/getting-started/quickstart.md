...
This runs the full pipeline: **ingest → static → normalize → graph → translate → statespace → process → export → validate**. The `--layout-output` flag reorganizes the results into `data/`, `diagrams/`, `site/`, `reports/`, and `figures/` subdirectories so the output tree is easy to browse. The batch runner `run_all.py` (see [`README.md`](https://github.com/cogant-contributors/cogant/blob/main/cogant/README.md)) adds `analysis/`, `exports/`, `gnn_package/`, and `roundtrip/` for the same target.
