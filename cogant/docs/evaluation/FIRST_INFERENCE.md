...
The roundtrip is not byte-identical and should not be described as strict structural isomorphism under the current taxonomy. The supported claim is narrower: the same hidden-state, observation, and action roles survive the cycle, and the synthesized code runs, infers, and acts.
