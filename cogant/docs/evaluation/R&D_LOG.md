...
> **Source of truth for current metrics:** [`evaluation/METRICS.yaml`](https://github.com/docxology/cogant/blob/main/evaluation/METRICS.yaml)
> (auto-generated by `tools/regenerate_metrics.py`).
>
> **Current state:** see `evaluation/METRICS.yaml` for live test counts, coverage, and
> roundtrip metrics. Current v0.6 taxonomy classifies the checked-in 23-row
> roundtrip ledger as `STALE_LEGACY`; native role-score aggregates remain null
> until a refreshed ledger is generated. Strict structural isomorphism is
> reported separately.
>
...
- `manuscript/06_00_experimental_setup.md` plus `06_03` / `06_04` — rewritten with tables containing real numbers harvested from actual pipeline runs on the six fixtures (calculator, event_pipeline, flask_mini, flask_app, requests_lib, json_stdlib). Replaces the prior placeholder tables that hand-waved on sizes and timings.
- `manuscript/10_conclusion.md` — expanded from 5 shipped capabilities to 10, reflecting the actual P0/P1 deliverables (A/B/C/D matrices, dynamic pipeline, VFE/EFE, Markov blanket, real-world fixtures, Rust scaffold, provenance, plugins, review workflow, AII validator compliance).
...
**Open items going into P5 (historical snapshot):**
- Literature review stream (LITERATURE.md, RELATED_WORK.md) was still pending at this point — tracked as LIT stream, not a gated phase.
- At this snapshot, manuscript figures were static PNGs. Current v0.6 outputs preserve that publication path but add figure manifests, per-figure sidecars, browser QA screenshots, dashboard evidence panels, and source-artifact digests; interactive web variants remain an optional future enhancement rather than a manuscript blocker.
...
### Deliverables
1. **Roundtrip role-preservation evaluation**: 23 targets (zoo/01–12 + 11 real-world repos); 8 ROLE_PRESERVED (s_role>=0.80), 9 DRIFT (0.50<=s_role<0.80), 6 FAILED (s_role<0.50)
2. **Empirical claim**: Full 10-step Active Inference cycle on zoo/01_simple_state (s_role=1.0, VFE=0.0 at each step); prior D=[1.0] → likelihood A=[1.0] → posterior [1.0] → policy u_c0 → transition [1.0]
...
- **ε metric**: multiset similarity (Jensen-Shannon on role distributions); a coverage-weighted metric or per-role recall alternative would improve FAILED recall without abandoning the Galois connection framing
...
- Roundtrip role-preservation metric
...
- `cogant.reverse.metrics` — VFE, KL divergence, roundtrip role preservation
...
- s_role=1.0 (exact roundtrip) on zoo/01_simple_state

**Roundtrip evaluation** (ROUNDTRIP_EVAL.md, 23 targets):
- 14 ROLE_PRESERVED (s_role>=0.80) — 61%
- 5 DRIFT (0.50<=s_role<0.80)
- 4 FAILED (s_role<0.50) — exclusively due to CONSTRAINT count collapse
...
- Historical role-distribution distance metric (Jensen-Shannon on role distributions; superseded in active docs by `role_preservation_score` plus `roundtrip_status`)
...
| 10-12 | 2026-04-10 | Real-world eval (8 repos), EMPIRICAL_CLAIM.md, roundtrip role preservation (23 targets), playground, notebooks (6), mkdocs full nav, ruff clean | 1634 | 76% |
...
| ROLE_PRESERVED targets (s_role>=0.80) | 0 | 14 (61%) | — |
...
| Roundtrip ROLE_PRESERVED | 19/23 (83%) |
...
### Summary verdict
COGANT is solid alpha/beta. The historical empirical claim (Active Inference cycle on real Python code, VFE=0.0, role-distribution roundtrip preservation validated on 19/23 targets in that snapshot) is reproducible and documented; active docs now report `roundtrip_status` and `role_preservation_score` instead of calling role-only preservation structural isomorphism. The engineering foundation (1873 tests, mypy strict enforced in CI, 82.6% coverage) is production-ready for non-edge-case repos. The dulwich scaling cliff and missing POLICY/CONTEXT synthesis are documented limitations, not correctness issues. v1.0 is achievable with 1–2 weeks of focused work on the scaling fix.
...
| Roundtrip role-preservation benchmark | Historical 23/23 (100%) — s_role = 1.0; current v0.6 metrics require native ledger refresh |
| Version | **v0.5.0** |

**Key discoveries at session start:**
- METRICS.yaml was stale from wave-14 era (2129 tests, 14/23 ROLE_PRESERVED)
- Wave-16 benchmark refresh DID run (commit b721b29) and reported 23/23 role-preserved rows, but the checked-in ledger now requires a native v0.6 refresh before serving as release evidence
...
- **Implementing modules (forward):** [`py/cogant/translate/`](https://github.com/docxology/cogant/blob/main/py/cogant/translate/), [`py/cogant/statespace/`](https://github.com/docxology/cogant/blob/main/py/cogant/statespace/), [`py/cogant/gnn/`](https://github.com/docxology/cogant/blob/main/py/cogant/gnn/), [`py/cogant/markov/`](https://github.com/docxology/cogant/blob/main/py/cogant/markov/), [`py/cogant/static/`](https://github.com/docxology/cogant/blob/main/py/cogant/static/)
- **Implementing modules (reverse):** [`py/cogant/reverse/`](https://github.com/docxology/cogant/blob/main/py/cogant/reverse/)
