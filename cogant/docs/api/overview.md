...
- **Zoo:** [`examples/zoo/01_simple_state/`](../../examples/zoo/01_simple_state/) — simplest end-to-end target for `Session.from_target` → `extract_static` → `translate_to_gnn` → `export_all`.
- **Zoo:** [`examples/zoo/04_pomdp_minimal/`](../../examples/zoo/04_pomdp_minimal/) — minimal POMDP that exercises all four `Session` analysis stages on a single fixture.
