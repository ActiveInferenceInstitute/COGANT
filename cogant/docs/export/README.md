...
> JSON/GraphML/Parquet-style consumers, and the feature-engineering,
> compression, validation, and reproducibility policies that govern them.
> Native PyTorch Geometric and DGL adapters are planned, not shipped.
> Read this
...
| [PyTorch Geometric export](pytorch_geometric_export.md) | Planned `torch_geometric.data.Data` contract; use JSON/Parquet exports today | Intermediate |
| [DGL export](dgl_export.md) | Planned `DGLGraph` contract; use JSON/Parquet exports today | Intermediate |
...
4. If you are planning a graph-learning adapter, read the planned
   [PyTorch Geometric](pytorch_geometric_export.md) or
   [DGL](dgl_export.md) contracts, then build from the shipped JSON/Parquet
   artifacts until native helpers exist.
