"""Targeted unit tests for ``cogant.cli.main``.
...
* ``version`` command.
* ``analyze-static``, ``analyze-graph``, ``visualize``, ``export`` — real
  commands that write analysis/visualization/export artifacts.
...
# ------------------------------------------------------------------ #
# Real analysis / visualization commands
# ------------------------------------------------------------------ #
...
def test_analyze_graph_stub(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["analyze-graph", str(target), "--output", str(out)])
    assert result.exit_code == 0
    assert out.is_file()
...
def test_visualize_stub(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["visualize", str(target), "--output", str(out)])
    assert result.exit_code == 0
    assert out.is_file()
...
def test_export_stub(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["export", str(target), "--output-dir", str(out_dir)])
    assert result.exit_code == 0
