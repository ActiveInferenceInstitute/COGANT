#!/usr/bin/env python3
"""Coverage boost batch 31 — config modules, cache/hasher, cache/store.
...
class TestCacheStore:
    ...
    def test_put_returns_entry(self, tmp_path):
        from cogant.cache.store import CacheStore, CacheEntry
        store = CacheStore(cache_dir=tmp_path)
        key = self._make_key()
    ...
    def test_serialize_deserialize_roundtrip(self, tmp_path):
        from cogant.cache.store import CacheStore, CacheKey, CacheEntry
        store = CacheStore(cache_dir=tmp_path)
