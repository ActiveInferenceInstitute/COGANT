"""Targeted unit tests for ``cogant.server.app`` and ``cogant.statespace.compiler``.
...
targeted coverage scan so the gains are deliberate and measurable.
...
# ============================================================================
# server.app — _RateLimiter behaviour (besides earlier coverage tests)
# ============================================================================
...
class TestRoundtripErrorPaths:
    ...
    def test_roundtrip_with_empty_dir_returns_5xx_or_200(
        ...
        empty_dir = tmp_path / "empty_repo_targeted"
        empty_dir.mkdir()
        r = client.post(
            "/roundtrip",
...
class TestApiV1AnalyzeError:
    ...
    def test_unknown_stage_returns_5xx_or_200(
        ...
        r = client.post(
            ...
            json={
                "repo_path": str(tiny_repo),
                "stages": ["does_not_exist_targeted"],
                "skip_dynamic": True,
            },
        )
        assert r.status_code in (200, 500)
...
class TestMiddlewareExceptionSafetyNet:
    def test_safety_net_converts_unhandled_exception_to_500(self) -> None:
        ...
        @app.get("/_blowup_targeted")
        async def blowup() -> dict:
            raise RuntimeError("boom from targeted")
...
class TestCompileSmoke:
    ...
    exercised through the targeted import path. This insulates the targeted
    test file from breakage in pre-existing earlier targeted fixtures while
