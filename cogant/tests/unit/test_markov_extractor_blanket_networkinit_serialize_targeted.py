#!/usr/bin/env python3
"""Targeted branch tests — markov blanket, normalize, and identity resolution.
...
def _make_rich_graph():
    ...
    perceive = builder.add_node(
        NodeKind.FUNCTION, "perceive", "mymodule.Agent.perceive", path="mymodule.py"
    )
    act = builder.add_node(NodeKind.FUNCTION, "act", "mymodule.Agent.act", path="mymodule.py")
    update = builder.add_node(
        NodeKind.FUNCTION, "update_state", "mymodule.Agent.update_state", path="mymodule.py"
    )
    helper = builder.add_node(NodeKind.FUNCTION, "helper", "mymodule.helper", path="mymodule.py")
...
class TestBlanketNetwork:
    def test_build_blanket_network_returns_network(self):
        from cogant.markov import BlanketNetwork, MarkovBlanketExtractor, build_blanket_network
...
class TestCanonicalNormalizer:
    ...
    def test_normalize_returns_none_for_unknown_type(self):
        ...
        norm = CanonicalNormalizer()
        fact = LanguageFact(fact_type="unknown_type_xyz", language="python", data={"name": "x"})
        result = norm.normalize(fact)
    ...
    def test_normalize_batch_multiple(self):
        ...
        facts = [
            LanguageFact(
                "function", "python", {"name": "f1", "qualified_name": "m.f1", "path": "m.py"}
            ),
            LanguageFact(
                "function", "python", {"name": "f2", "qualified_name": "m.f2", "path": "m.py"}
            ),
        ]
    ...
    def test_get_normalization_stats(self):
        ...
        norm = CanonicalNormalizer()
        fact = LanguageFact(
            "function", "python", {"name": "f", "qualified_name": "m.f", "path": "m.py"}
        )
        norm.normalize(fact)
