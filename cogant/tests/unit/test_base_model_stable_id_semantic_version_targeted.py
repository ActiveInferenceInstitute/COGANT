#!/usr/bin/env python3
"""Targeted branch tests: schemas modules — base, core, state_space, process_model,
...
class TestConfidenceMetric:
    ...
    def test_with_rationale(self):
        ...
        m = ConfidenceMetric(
            score=0.75, rationale="good evidence", evidence_types=["static_analysis"]
        )
...
class TestGenerateStableID:
    ...
    def test_returns_stable_id_type(self):
        from cogant.schemas.base import StableID, generate_stable_id
...
class TestObservationModality:
    def test_basic(self):
        ...
        om = ObservationModality(
            modality_id="obs1", name="camera", observation_type=_make_type_info()
        )
...
class TestLikelihood:
    def test_observation(self):
        ...
        lk = Likelihood(
            likelihood_id="lk1", kind="observation_likelihood", distribution_type="gaussian"
        )
...
class TestStateSpaceModel:
    def _make_state_space_model(self):
        from cogant.schemas.base import StableID
        from cogant.schemas.state_space import StateSpaceKind, StateSpaceModel, StateVariable

        return StateSpaceModel(
            model_id=StableID("ssm1"),
            kind=StateSpaceKind.DISCRETE,
            name="Test Model",
            state_variables=[StateVariable(var_id="v1", name="x", value_type=_make_type_info())],
        )
    ...
    def test_with_observations_actions(self):
        ...
        m = StateSpaceModel(
            ...
            observation_modalities=[
                ObservationModality(
                    modality_id="obs1", name="cam", observation_type=_make_type_info()
                ),
            ],
            actions=[
...
class TestSideEffect:
    def test_basic(self):
        ...
        se = SideEffect(effect_id="se1", description="writes to DB", effect_type="database_write")
...
class TestProcessStage:
    ...
    def test_full(self):
        from cogant.schemas.process_model import ProcessKind, ProcessStage, SideEffect, TriggerKind
...
class TestProcessModel:
    def test_basic(self):
        from cogant.schemas.base import StableID
        from cogant.schemas.process_model import ProcessKind, ProcessModel, ProcessStage
...
class TestSourceGraphElement:
    def test_basic(self):
        ...
        el = SourceGraphElement(
            element_id=StableID("fn1"),
            element_type="node",
            label="authenticate",
...
class TestTargetSemanticElement:
    def test_basic(self):
        from cogant.schemas.semantic_mapping import SemanticRole, TargetSemanticElement

        el = TargetSemanticElement(
            semantic_id="obs_auth",
...
class TestSemanticMapping:
    def _make_mapping(self):
        ...
        src = SourceGraphElement(
            element_id=StableID("fn1"),
            element_type="node",
            label="auth_check",
            element_kind="function",
        )
        tgt = TargetSemanticElement(
            semantic_id="obs1",
            role=SemanticRole.OBSERVATION,
            label="auth_status",
            model_type="pomdp",
...
class TestSemanticMappingCollection:
    def test_empty(self):
        ...
        c = SemanticMappingCollection(
            collection_id=StableID("col1"), program_graph_id=StableID("pg1")
        )
    ...
    def test_with_mappings(self):
        ...
        src = SourceGraphElement(
            element_id=StableID("fn1"), element_type="node", label="x", element_kind="function"
        )
        tgt = TargetSemanticElement(
            semantic_id="obs1", role=SemanticRole.ACTION, label="y", model_type="mdp"
        )
        m = SemanticMapping(
            mapping_id=StableID("m1"),
            source_graph_elements=[src],
            target_semantic_elements=[tgt],
            confidence=ConfidenceMetric(score=0.8),
        )
        c = SemanticMappingCollection(
            collection_id=StableID("col2"),
...
class TestValidationCheck:
    def test_basic(self):
        from cogant.schemas.validation import CheckStatus, ValidationCheck
    ...
    def test_failed_with_issues(self):
        from cogant.schemas.validation import CheckLevel, CheckStatus, ValidationCheck
...
class TestValidationMetrics:
    ...
    def test_full(self):
        ...
        m = ValidationMetrics(
            ...
            edge_count=1200,
            mapping_count=150,
            state_variable_count=20,
            process_stage_count=10,
...
class TestValidationReport:
    ...
    def test_full(self):
        ...
        vc = ValidationCheck(
            check_id="c1", name="Test", check_type="test", status=CheckStatus.PASSED
        )
        m = ValidationMetrics(node_count=100)
        rec = ValidationRecommendation(
            recommendation_id="r1", category="cov", title="T", description="D"
        )
        vr = ValidationReport(
            report_id=StableID("rpt2"),
            bundle_id="bnd2",
            checks=[vc],
            metrics=m,
            recommendations=[rec],
...
class TestProvenanceRecord:
    def test_source_span(self):
        ...
        r = ProvenanceRecord(kind=EvidenceKind.SOURCE_SPAN, uri="src/auth.py", element_id="fn_auth")
    ...
    def test_config_entry(self):
        ...
        r = ProvenanceRecord(
            kind=EvidenceKind.CONFIG_ENTRY,
            uri="config.yaml",
    ...
    def test_commit_event(self):
        ...
        r = ProvenanceRecord(
            kind=EvidenceKind.COMMIT_EVENT,
            uri="git://repo",
            commit_hash="abc123",
            commit_author="Alice",
...
class TestGNNMetadata:
    ...
    def test_with_framework(self):
        ...
        m = GNNMetadata(export_id="ex2", bundle_id="bnd2", gnn_framework="pytorch_geometric")
...
class TestFactorSection:
    def test_basic(self):
        ...
        f = FactorSection(factor_id="f1", factor_type="unary", variables=["v1"], cardinalities=[2])
...
class TestLikelihoodStructureSection:
    def test_basic(self):
        ...
        lss = LikelihoodStructureSection(
            kind="observation_likelihood", distribution_type="gaussian"
        )
...
class TestPreferenceConstraintSection:
    def test_basic(self):
        ...
        pcs = PreferenceConstraintSection(
...
class TestTimeSettingSection:
    ...
    def test_full(self):
        ...
        ts = TimeSettingSection(
            is_continuous_time=False, time_unit="steps", time_step=0.1, max_episode_length=1000
        )
...
class TestOntologyMappingSection:
    def test_basic(self):
        ...
        om = OntologyMappingSection(
            mapping_id="m1", source_element_id="fn1", target_semantic_role="observation"
        )
...
class TestRenderingHints:
    ...
    def test_full(self):
        ...
        rh = RenderingHints(
            recommended_layout="force-directed",
...
class TestGNNExportBundle:
    ...
    def test_with_graph(self):
        ...
        b = GNNExportBundle(
            ...
            connections=[
                ConnectionSection(source_node_index=0, target_node_index=1, edge_type="calls")
            ],
            ...
            factors=[
                FactorSection(
                    factor_id="f1", factor_type="unary", variables=["v1"], cardinalities=[2]
                )
            ],
            ...
            ontology_mapping=[
                OntologyMappingSection(
                    mapping_id="m1", source_element_id="fn1", target_semantic_role="obs"
                )
            ],
...
class TestTargetInfo:
    def test_basic(self):
        ...
        ti = TargetInfo(name="myproject", version="1.0.0", primary_language=TargetLanguage.PYTHON)
...
class TestProvenanceOrigin:
    ...
    def test_full(self):
        ...
        po = ProvenanceOrigin(
            analyzer_name="cogant",
            analyzer_version="0.4.0",
            ingest_host="myhost.local",
            ingest_user="alice",
