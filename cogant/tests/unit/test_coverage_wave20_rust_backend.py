"""Wave-20 coverage tests for ``cogant.rust_backend``.
...
def test_build_program_graph_python_when_rust_unavailable(monkeypatch) -> None:
    """Even with use_rust=True, the pure-Python builder is used when
    RUST_AVAILABLE is False (the runtime fallback safety net)."""
    monkeypatch.setattr(rust_backend, "RUST_AVAILABLE", False)
    builder = build_program_graph("repo://x", use_rust=True)
    from cogant.graph.builder import ProgramGraphBuilder
