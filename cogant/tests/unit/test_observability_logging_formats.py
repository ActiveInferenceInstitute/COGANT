"""Targeted unit tests for cogant.observability.logging.
...
def test_setup_logging_json_format_configures_structlog():
    ...
    logger = log_mod.get_logger("targeted.json")
    assert logger is not None
...
def test_setup_logging_unknown_format_falls_into_json_branch():
    ...
    assert log_mod.get_logger("targeted.unknown") is not None
...
def test_get_logger_returns_logger_with_name():
    ...
    logger = get_logger("targeted.named")
    assert logger is not None
