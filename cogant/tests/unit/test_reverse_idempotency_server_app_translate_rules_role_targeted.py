#!/usr/bin/env python3
"""Targeted branch tests — idempotency helpers, server probes, translate rules.
...
class TestSemanticRulesMatches:
    ...
    def test_context_rule_name(self):
        try:
            from cogant.translate.rules.semantic import ContextRule

            rule = ContextRule()
            assert isinstance(rule.name, str)
        except ImportError:
            pytest.fail("ContextRule should be importable")
    ...
    def test_context_rule_matches_returns_list(self):
        try:
            from cogant.graph.queries import GraphQuery
            from cogant.translate.rules.semantic import ContextRule
    ...
    def test_preference_rule_matches_returns_list(self):
        try:
            from cogant.graph.queries import GraphQuery
            ...
            pytest.fail("PreferenceRule should be importable")
