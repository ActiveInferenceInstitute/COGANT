#!/usr/bin/env python3
"""Targeted branch tests — translate/rules/structural.py and resilience.py.
...
class TestSingletonAccessRule:
    def test_name_property(self):
        try:
            from cogant.translate.rules.resilience import SingletonAccessRule

            rule = SingletonAccessRule()
            assert isinstance(rule.name, str)
        except (ImportError, AttributeError):
            pytest.fail("SingletonAccessRule should be available")
