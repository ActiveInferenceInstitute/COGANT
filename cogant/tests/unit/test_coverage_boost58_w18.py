#!/usr/bin/env python3
"""Coverage boost batch 58 — markov/blanket.py, markov/extractor.py,
markov/network.py.
...
class TestMarkovBlanket:
    def _make_blanket(self):
        from cogant.markov.blanket import MarkovBlanket, BlanketRole
        return MarkovBlanket(
            roles={"n0": BlanketRole.INTERNAL, "n1": BlanketRole.SENSORY, "n2": BlanketRole.EXTERNAL},
            seeds={"n0"},
            internal_ids={"n0"},
            sensory_ids={"n1"},
            active_ids=set(),
...
class TestSerializeBlanket:
    def test_serialize_empty_blanket(self):
        from cogant.markov.blanket import serialize_blanket, partition_by_seeds
        graph = _make_empty_graph()
        blanket = partition_by_seeds(graph, seeds=set())
    ...
    def test_serialize_with_nodes(self):
        ...
        assert "internal" in result or "sensory" in result or "external" in result or "roles" in result
