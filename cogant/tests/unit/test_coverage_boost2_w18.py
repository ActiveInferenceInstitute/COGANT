#!/usr/bin/env python3
"""Wave-18 coverage boost tests — batch 2.
...
class TestIdentityResolver:
    ...
    def test_generate_id_is_deterministic(self) -> None:
        ...
        id2 = r.generate_id("module", "repo://demo", path="src/main.py")
...
class TestModelRunnerValidate:
    ...
    def _make_ss_with_var(self, var_id: str, var_name: str, kind_name: str = "DISCRETE", dim: int = 2):
        ...
        ss.variables[var_id] = StateVariable(id=var_id, name=var_name, var_type=kind, node_id=var_id)
        return ss
    ...
    def test_validate_model_with_dangling_action_reference(self) -> None:
        from cogant.statespace.compiler import Action, Transition
        runner = self._make_runner()
    ...
    def test_validate_model_counts_correctly(self) -> None:
        ...
        runner = self._make_runner()
        ss = self._make_ss_with_var("v1", "x")
        ss.variables["v2"] = StateVariable(id="v2", name="y", var_type=StateVariableType.DISCRETE, node_id="v2")
    ...
    def test_simulate_step_increments_integer(self) -> None:
        ...
        ss.actions["inc"] = Action(id="inc", name="increment", controller_id="ctrl", effects=["count"])
        state = {"count": 5}
...
class TestEnrichmentHelpers:
    ...
    def test_node_spans_line_within_range(self) -> None:
        ...
        from cogant.schemas.core import Node, NodeKind
        node = Node(
            id="n1", kind=NodeKind.FUNCTION, name="fn", qualified_name="fn",
