"""Targeted unit tests for: cogant package ``__init__`` public entry point.
...
class TestFirstPartyImportFailsLoudly:
    """First-party submodule import failures must fail loudly, not silently
    set the public name to ``None``.

    Before 2026-05-19 (RedTeam F15) the package wrapped each first-party
    import in ``try/except (ImportError, ModuleNotFoundError)`` and
    silently bound the name to ``None`` — but the matching ``.pyi``
    declared them as concrete classes, so mypy-trusting consumers could
    hit ``AttributeError: 'NoneType' object has no attribute`` at
    runtime. The honest contract now is: first-party module
    unavailability is an installation defect, not a runtime fallback.
    These tests pin that contract.

    The Rust extension is the *one* legitimate optional dep; it retains
    its fallback (``_RUST_AVAILABLE = False`` on import failure) and
    the matching test below.
    """

    def test_session_import_fails_loudly_when_api_session_missing(
        self, restore_cogant: object
    ) -> None:
        with pytest.raises((ImportError, ModuleNotFoundError)):
            _reload_cogant_with_blocked({"cogant.api.session"})

    def test_pipeline_runner_import_fails_loudly_when_pipeline_missing(
        self, restore_cogant: object
    ) -> None:
        with pytest.raises((ImportError, ModuleNotFoundError)):
