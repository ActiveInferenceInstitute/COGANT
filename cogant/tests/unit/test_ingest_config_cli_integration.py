"""Targeted unit tests for ingest.manifest, config.loaders, and cli.main.
...
cross-references a specific uncovered line range from the targeted
coverage scan so the gains are deliberate and measurable.

Targets (and pre-parameter/state/rate-limit coverage):
...
class TestRealArtifactCommands:
    """analyze-static, analyze-graph, visualize, and export write artifacts."""
    ...
    def test_visualize_runs(self, runner: CliRunner, tmp_path: Path) -> None:
        ...
        result = runner.invoke(app, ["visualize", str(tmp_path), "--output", str(out)])
        assert result.exit_code == 0
        assert out.is_file()
    ...
    def test_export_cmd_runs(self, runner: CliRunner, tmp_path: Path) -> None:
        ...
        result = runner.invoke(app, ["export", str(tmp_path), "--output-dir", str(out_dir)])
        assert result.exit_code == 0
        assert (out_dir / "exports_manifest.json").is_file()
