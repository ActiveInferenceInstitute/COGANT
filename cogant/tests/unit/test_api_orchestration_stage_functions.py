...
def test_graph_marks_test_generated_and_async_call_semantics(tmp_path: Path) -> None:
    ...
    (repo / "generated.py").write_text(
        "# Generated by fixture compiler; DO NOT EDIT.\ndef generated_value():\n    return 1\n",
        encoding="utf-8",
    )
    ...
    test_nodes = [node for node in nodes if node.kind.value == "test"]
    generated_nodes = [node for node in nodes if node.metadata.get("is_generated") is True]
    async_edges = [edge for edge in graph.edges.values() if edge.kind.value == "async_calls"]
    trigger_edges = [edge for edge in graph.edges.values() if edge.kind.value == "triggers"]

    assert any(node.name == "test_flow" for node in test_nodes)
    assert generated_nodes
    assert async_edges
    assert async_edges[0].metadata["async"] is True
