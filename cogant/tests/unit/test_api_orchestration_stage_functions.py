"""Targeted unit tests for: exercise cogant.api.orchestration end-to-end.
...
def _module_node_ids(result: dict[str, object]) -> dict[str, str]:
    nodes = result["nodes"]
    assert isinstance(nodes, dict)
    return {
        node["qualified_name"]: node_id
        for node_id, node in nodes.items()
        if isinstance(node, dict) and node.get("kind") == "module"
    }


def _import_targets_for_module(result: dict[str, object], module_name: str) -> set[str]:
    module_ids = _module_node_ids(result)
    edges = result["edges"]
    assert isinstance(edges, dict)
    source_id = module_ids[module_name]
    return {
        edge["target_id"]
...
class TestRunGraph:
    ...
    def test_graph_emits_imports_edge_for_in_repo_from_import(self, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
        (repo / "helper.py").write_text("def compute(x):\n    return x * 2\n")
        (repo / "main.py").write_text(
        ...
        module_ids = _module_node_ids(result)
        assert _import_targets_for_module(result, "main") == {module_ids["helper"]}

    def test_graph_resolves_supported_import_shapes_without_dangling_edges(
        self, tmp_path: Path
    ) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
        (repo / "addon.py").write_text("VALUE = 1\n")
        (repo / "helper.py").write_text("def compute(x):\n    return x * 2\n")
        (repo / "inner.py").write_text("NAME = 'inner'\n")
        (repo / "main.py").write_text(
            "import helper\n"
            "import addon.util\n"
            "from helper import compute\n"
            ...
            "\n"
            "def run(value):\n"
            "    return compute(value) + helper.compute(value)\n"
        )
        ...
        module_ids = _module_node_ids(result)
        assert _import_targets_for_module(result, "main") == {
            module_ids["addon"],
            module_ids["helper"],
            module_ids["inner"],
        }
