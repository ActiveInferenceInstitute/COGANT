...
def _make_trace():
    ...
    return [
        {
            "step": 0, "state": {"x": "s0"}, "action": "move_right",
            "observation": "obs_0", "reward": 1.0,
            "beliefs": {"state_a": 0.8, "state_b": 0.2},
            "free_energy": 2.0,
            "policy_scores": [("move_right", 0.9), ("stay", 0.1)],
        },
        {
            "step": 1, "state": {"x": "s1"}, "action": "stay",
            "observation": "obs_1", "reward": 0.5,
            "beliefs": {"state_a": 0.5, "state_b": 0.5},
            "free_energy": 1.5,
        },
        {
            "step": 2, "state": {"x": "s0"}, "action": "move_left",
            "observation": "obs_0", "reward": 0.2,
...
class TestSimulationVisualizer:
    ...
    def test_fe_trajectory_constant_fe(self):
        ...
        v = self._viz()
        trace = [
            {"free_energy": 1.0, "step": i, "state": {}} for i in range(5)
        ]
        result = v.plot_free_energy_trajectory(trace)
        assert "<svg" in result
    ...
    def test_belief_evolution_many_states(self):
        ...
        trace = [
            {"beliefs": {f"s{i}": 1.0 / 10 for i in range(10)}}
            for _ in range(5)
        ]
        result = v.plot_belief_evolution(trace)
        assert "<svg" in result
    ...
    def test_html_report_with_fe_stats(self):
        v = self._viz()
        trace = [{"free_energy": float(i), "beliefs": {}, "action": "stay"}
                 for i in range(5)]
...
class TestGNNJSONExporterImport:
    ...
    def test_init(self):
        ...
        from cogant.schemas.graph import GraphMetadata
