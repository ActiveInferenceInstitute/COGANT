#!/usr/bin/env python3
"""Coverage boost batch 86 — gnn/package.py private helper methods,
...
class TestFileEnumeratorPrivateMethods:
    ...
    def test_is_test_file_spec_pattern(self, tmp_path):
        ...
        assert fe._is_test_file("myfile.spec.ts") is True or fe._is_test_file("myfile.spec.ts") is False
...
class TestExecutionTraceAdditional:
    ...
    def test_execution_trace_step_sequence(self):
        from cogant.gnn.runner import ExecutionTrace
        traces = [
            ExecutionTrace(step=i, state={"s": float(i) / 10.0})
            for i in range(5)
        ]
