"""Targeted unit tests for cogant.reverse.planner.
...
def test_to_json_returns_valid_json_string():
    ...
    assert parsed["target_role_counts"] == {
        "ACTION": 1,
        "HIDDEN_STATE": 1,
        "OBSERVATION": 1,
    }
...
def test_from_json_round_trip_preserves_structure():
    ...
    assert restored.target_role_counts == original.target_role_counts
...
def test_plan_package_without_observations_avoids_fixed_scaffolds():
    """Default plans do not add fixed policy/context scaffolds."""
    model = ReverseGNNModel(
        model_name="bare",
        hidden_states=["s_f0"],
    )
    plan = plan_package(model)
    assert plan.target_role_counts == {"HIDDEN_STATE": 1}
    assert plan.scaffold_policy_functions == []
    assert plan.scaffold_context_classes == []


def test_plan_package_expected_roles_emit_deficit_scaffolds():
    """Caller-supplied targets drive policy/constraint/context deficits."""
    model = ReverseGNNModel(
        model_name="targeted",
        hidden_states=["s_f0"],
        observations=["o_m0"],
    ...
    assert plan.target_role_counts["POLICY"] == 3
    assert len(plan.scaffold_policy_functions) == 2  # selector helper supplies one
    assert len(plan.scaffold_constraint_checks) == 1
    assert len(plan.scaffold_context_classes) == 1
