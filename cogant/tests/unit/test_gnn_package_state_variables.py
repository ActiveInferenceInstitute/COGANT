"""Targeted GNN package tests targeting:
...
class TestPackageBuilderMarkdownAndJsonFailures:
    ...
    def test_generate_markdown_raises_propagates(self, tmp_path: Path) -> None:
        ...
        builder = _make_builder()
        method_globals = GNNPackageBuilder._generate_markdown.__globals__
        original = method_globals["GNNMarkdownFormatter"]
        ...
        method_globals["GNNMarkdownFormatter"] = _RaisingFormatter
        try:
            with pytest.raises(RuntimeError, match="formatter exploded"):
                builder._generate_markdown(tmp_path)
        finally:
            method_globals["GNNMarkdownFormatter"] = original
    ...
    def test_generate_json_model_raises_propagates(self, tmp_path: Path) -> None:
        ...
        builder = _make_builder()
        method_globals = GNNPackageBuilder._generate_json_model.__globals__
        original = method_globals["GNNJSONExporter"]
