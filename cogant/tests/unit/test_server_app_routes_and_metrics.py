"""Targeted unit tests for: cogant.server.app.

Targets uncovered branches in the FastAPI server that
``test_server_app_http_routes.py`` and ``test_server_api_v1.py`` do not
exercise:
...
Style mirrors ``test_server_app_http_routes.py`` and
...
class TestApiV1RoundtripHappy:
    def test_roundtrip_returns_response_with_request_id_and_score(
        ...
        if r.status_code == 200:
            body = r.json()
            assert "request_id" in body
            assert "role_preservation_score" in body
            assert "roundtrip_status" in body
...
# --------------------------------------------------------------------------- #
# _MetricsStore happy-path (already touched in earlier server tests, but exercise more)
# --------------------------------------------------------------------------- #
