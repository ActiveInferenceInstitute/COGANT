"""Targeted branch tests for ``cogant.gnn.json_export``.
...
def _build_program_graph(custom_meta: dict[str, Any] | None = None) -> ProgramGraph:
    ...
    builder = ProgramGraphBuilder(repo_uri="test://targeted-export")
...
class TestNumericConfidenceSuccessPath:
    ...
    def test_average_confidence_with_numeric_values(self) -> None:
        ...
        # Numeric values were appended; mean of {0.85, 0.6, 0.75, 0.9} = 0.775
        assert avg == pytest.approx(0.775)
    ...
    def test_average_confidence_with_categorical_values(self) -> None:
        graph = _build_program_graph()
        var = StateVariable(
            id="v1",
            name="v1",
        ...
        exporter = GNNJSONExporter(
            program_graph=graph,
            state_space_model=ss,
            process_model=_build_process_model(),
            semantic_mappings={},
        )
        assert exporter._compute_average_confidence() == pytest.approx(0.85)
        assert exporter._compute_component_confidence("variables") == pytest.approx(0.85)
