...
def test_render_program_graph_png_writes_file(tmp_path):
    ...
    sidecar = out.with_suffix(".figure.json")
    assert sidecar.is_file()
    metadata = json.loads(sidecar.read_text())
    assert metadata["displayed_counts"]["nodes"] == 3
    assert metadata["displayed_counts"]["edges"] == 2


def test_render_program_graph_png_reads_source_id_edges_and_rule_roles(tmp_path):
    """Real package graphs use source_id/target_id and rule trace matched_node_ids."""
    pg = tmp_path / "program_graph.json"
    pg.write_text(
        json.dumps(
            {
                "nodes": {
                    "m": {"id": "m", "name": "module", "kind": "module"},
                    "c": {"id": "c", "name": "Controller", "kind": "class"},
                },
                "edges": {
                    "e1": {"id": "e1", "source_id": "m", "target_id": "c", "kind": "contains"}
                },
            }
        )
    )
    (tmp_path / "rule_evidence_trace.json").write_text(
        json.dumps(
            {
                "mappings": [
                    {
    ...
    assert metadata["edge_kind_counts"] == {"contains": 1}
    assert metadata["semantic_role_counts"] == {"hidden_state": 1}
...
def test_render_state_space_factor_png_writes_file(tmp_path):
    ...
    assert out.is_file()
    metadata = json.loads(out.with_suffix(".figure.json").read_text())
    assert metadata["displayed_counts"]["hidden_states"] == 2
    assert metadata["displayed_counts"]["observations"] == 1
    assert metadata["displayed_counts"]["actions"] == 1
    assert metadata["panels"][0]["key"] == "factor_graph"
...
def test_render_interpretability_overview_png_with_artifacts(tmp_path):
    """Interpretability overview renders from graph, mappings, state-space, blanket."""
    _write_program_graph(tmp_path / "program_graph.json")
    (tmp_path / "semantic_mappings.json").write_text(
        json.dumps(
            {
                "mappings": [
                    {"id": "m1", "kind": "observation"},
                    {"id": "m2", "kind": "action"},
                ]
            }
        )
    ...
    out = tmp_path / "interpretability.png"
    assert render_interpretability_overview_png(tmp_path, out) is True
    assert out.is_file()
